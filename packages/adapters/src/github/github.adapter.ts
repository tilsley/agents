...
export class GitHubAdapter implements GitHubPort {
  ...
  async rerunCheckRun(owner: string, repo: string, checkRunId: number): Promise<void> {
    await this.octokit.checks.rerequestRun({ owner, repo, check_run_id: checkRunId });
  }
  ...
  async closePullRequest(
    ...
    await Promise.all([
      this.octokit.issues.createComment({ owner, repo, issue_number: prNumber, body: comment }),
      this.octokit.pulls.update({ owner, repo, pull_number: prNumber, state: "closed" }),
    ]);
  }
